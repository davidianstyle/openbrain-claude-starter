...
- **CLAUDE.md changes are always human-approved, per instance — no stand-ins.** The vault version has fully resolved tables and user-specific content. Only port structural changes (new sections, reworded rules, new skill entries in tables). Never overwrite the resolved routing tables with `{{PLACEHOLDER}}` markers. Session-level authorization, standing approvals, and "this matches a previously-approved change" do not substitute for asking: every CLAUDE.md change gets its own explicit approval from the user before it is applied. An **unattended or scheduled run halts-and-flags at any CLAUDE.md decision** — it applies nothing further in that step, leaves the state honest (one step behind is acceptable; an unconfirmed CLAUDE.md change is not), and surfaces the pending decision loudly in its report. Rationale: CLAUDE.md is an instruction-injection surface for every future session — an auto-applied change executes as standing instructions in every session thereafter, and this template encourages scheduled/nightly runs, so it gets human eyes every time.
