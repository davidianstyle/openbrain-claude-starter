...
- **Auto git sync hooks** (opt-in, configured during setup):
  ...
  - **Stop hook** (`.openbrain/on-stop.sh`) — regenerates Home.md MOC index, then smart-commits all changes and pushes (skip-if-clean, pull-rebase first, conflict → inbox note). Set `OPENBRAIN_AUTOPUSH=0` in the hook's environment (e.g. in `.claude/settings.json`) to keep commits local-only — useful when `origin` is a repo that must not receive vault content, such as a public template fork.
